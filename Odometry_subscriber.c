...
void
nav_msgs_msg_OdometrySubscriber_on_data_available(void *listener_data,
DDS_DataReader * reader)
{
    nav_msgs_msg_OdometryDataReader *hw_reader = nav_msgs_msg_OdometryDataReader_narrow(reader);
    DDS_ReturnCode_t retcode;
    struct DDS_SampleInfo *sample_info = NULL;
    nav_msgs_msg_Odometry *sample = NULL;

    struct DDS_SampleInfoSeq info_seq = 
    DDS_SEQUENCE_INITIALIZER;
    struct nav_msgs_msg_OdometrySeq sample_seq = 
    DDS_SEQUENCE_INITIALIZER;

    const DDS_Long TAKE_MAX_SAMPLES = 32;
    DDS_Long i;

    retcode = nav_msgs_msg_OdometryDataReader_take(hw_reader, 
    ...
    for (i = 0; i < nav_msgs_msg_OdometrySeq_get_length(&sample_seq); ++i)
    {
        sample_info = DDS_SampleInfoSeq_get_reference(&info_seq, i);

        if (sample_info->valid_data)
        {
            sample = nav_msgs_msg_OdometrySeq_get_reference(&sample_seq, i);
    ...
    nav_msgs_msg_OdometryDataReader_return_loan(hw_reader, &sample_seq, &info_seq);

    done:
    nav_msgs_msg_OdometrySeq_finalize(&sample_seq);
    DDS_SampleInfoSeq_finalize(&info_seq);
}

void
nav_msgs_msg_OdometrySubscriber_on_subscription_matched(void *listener_data,
DDS_DataReader * reader,
const struct
...
*/
void
nav_msgs_msg_OdometrySubscriber_deserialize_unsigned_long(char **src_buffer,
    ...
    DDS_Boolean *dropped)
{
    ...
    *
    * NOTE: If you want to use a different field for filtering (e.g. your type does
    * not have a field called id as first field), you will need to reimplement this
    * function and nav_msgs_msg_OdometrySubscriber_deserialize_unsigned_long
    * to match your type.
    */
    nav_msgs_msg_OdometrySubscriber_deserialize_unsigned_long(&src_buffer,
    need_byte_swap,
    (DDS_UnsignedLong*)&id);
...
/*i
* \brief Helper function to filter an nav_msgs_msg_Odometry sample
*
* \param[in]  sample       A nav_msgs_msg_Odometry data sample to filter
* \param[out] drop_sample  Out parameter determining whether the sample
*                          should be filtered out or not.
*/
void 
nav_msgs_msg_OdometrySubscriber_filter_sample(nav_msgs_msg_Odometry *sample,
...
*/
DDS_Boolean
nav_msgs_msg_OdometrySubscriber_on_before_sample_commit(
    void *listener_data,
    DDS_DataReader *reader,
    ...
    DDS_Boolean *dropped)
{
    nav_msgs_msg_Odometry *hw_sample = (nav_msgs_msg_Odometry *)sample; 

    nav_msgs_msg_OdometrySubscriber_filter_sample(hw_sample, dropped);
...
DDS_Long sleep_time, DDS_Long count)
{
    ...
    */
    ...
    dr_listener.on_data_available = nav_msgs_msg_OdometrySubscriber_on_data_available;
    dr_listener.on_subscription_matched =
    nav_msgs_msg_OdometrySubscriber_on_subscription_matched;
