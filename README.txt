...
=====================
C Odometry w/ Dynamic Discovery Example
=====================
...
A simple "Odometry" type, containing a message string, is defined in 
Odometry.idl.
...
can be invoked manually, with an example command like this:
    ...
    ${RTIMEHOME}/rtiddsgen/scripts/rtiddsgen -micro -language C Odometry.idl

The generated source files are Odometry.c, OdometrySupport.c, and 
OdometryPlugin.c. Associated header files are also generated.
 
The DataWriter and DataReader of the type are managed in Odometry_publisher.c
and Odometry_subscriber.c, respectively. The DomainParticipant of each is 
managed in OdometryApplication.c
...
Odometry_publisher.c:
This file contains the logic for creating a Publisher and a DataWriter, and 
sending data.  

Odometry_subscriber.c:
This file contains the logic for creating a Subscriber and a DataReader, a 
DataReaderListener, and listening for data.

OdometryPlugin.c:
This file creates the plugin for the Odometry data type.  This file contains 
the code for serializing and deserializing the Odometry type, creating, 
copying, printing and deleting the Odometry type, determining the size of the 
serialized type, and handling hashing a key, and creating the plug-in.

OdometrySupport.c
This file defines the Odometry type and its typed DataWriter, DataReader, and 
Sequence.

Odometry.c
This file contains the APIs for managing the Odometry type.
...
Linux
-----
cd "<OdometryApplication directory>"
...
------------------------------------------------------
Running Odometry_publisher and Odometry_subscriber
------------------------------------------------------

E.g. in case the Odometry has been compiled for Linux i86Linux2.6gcc4.4.5 run the subscriber by typing:

objs/i86Linux2.6gcc4.4.5/Odometry_subscriber -domain <Domain_ID> -peer <address> -sleep <sleep_time> -count <seconds_to_run>

and run the publisher by typing:

objs/i86Linux2.6gcc4.4.5/Odometry_publisher -domain <Domain_ID> -peer <address> -sleep <sleep_time> -count <seconds_to_run>
